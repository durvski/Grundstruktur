...
for j = 1:size(A,1)-4
  ...
  plot(pts(1,:), pts(2,:), 'm--', 'LineWidth', 1.5)%draw hyperplanes
end
if ~isempty(A)
  V = lcon2vert(A, b);% 
  k = convhull(V(:,1), V(:,2));
...
y = [cos(th);sin(th)];
x = bsxfun(@plus, C*y, d);
plot(x(1,:), x(2,:), 'b-', 'LineWidth', 2);%draw ellipse
plot([lb(1),ub(1),ub(1),lb(1),lb(1)], [lb(2),lb(2),ub(2),ub(2),lb(2)], 'k-');%draw boundary

pad = (ub - lb) * 0.05;
xlim([lb(1)-pad(1),ub(1)+pad(1)])%extension
