...
% inverse mapping of the vertices of the obstacles
  ...
  obs_image_dists = min(image_dists, [], 1);    % shortest distance from the four vertices of each obstacle
  [~, obs_sort_idx] = sort(obs_image_dists);  % sorting the obstacles with the closest vertices 

  flat_obs_pts = reshape(obstacle_pts, dim, []);

  A = zeros(n_obs,dim);
  b = zeros(n_obs,1);
    % start from the closest obstacle
  for i = obs_sort_idx;
    if uncovered_obstacles(i)
      ...
      dists = image_dists(:,i);
      [~,idx] = min(dists);
      xi = obs(:,idx);  % xi is xstar in the paper
      nhat = 2 * Cinv2 * (xi - d);
      nhat = nhat / norm(nhat);
      b0 = nhat' * xi;
      if all(nhat' * obs - b0 >= 0) % check if xi the closest point on the obstacles
        ...
        % image point on the obstacle with least distance
        if norm(ystar) < 1e-3
          ...
          xstar = C*ystar + d;  % xstar the point on the obstacle(i) closest to the ellipse
      ...
      check = A(i,:) * flat_obs_pts >= b(i);    %check which obstacle is separated by the current hyperplane based on the vertices
